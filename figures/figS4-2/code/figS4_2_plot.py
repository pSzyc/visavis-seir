...
for altered_parameter, fold_change in product(altered_parameters, fold_changes):
    ...
    coefs = coefficients.loc[altered_parameter, fold_change]

    propensities = pd.read_csv(data_dir / altered_parameter / str(fold_change) / 'fig2C--propensities.csv').fillna(0)
    ...
    plt.savefig(data_dir / altered_parameter / str(fold_change) / 'fig2C.png')
    plt.savefig(data_dir / altered_parameter / str(fold_change) / 'fig2C.svg')
    plt.close(fig)
...
for altered_parameter, data in coefficients.groupby('altered_parameter'):
    for ax, coef_name in zip(axs, data.columns):
        ax.plot(data.index.get_level_values('fold_change'), data[coef_name], 'o-', label=altered_parameter)
        ax.set_xlabel('fold_change')
        ax.set_ylabel(coef_name)
        ax.legend()

plt.savefig(out_dir / 'figS4-2.svg')
plt.savefig(out_dir / 'figS4-2.png')
...
for altered_parameter, data in equal_reasons_w.groupby('altered_parameter'):
    ax.plot(data.index.get_level_values('fold_change'), data, 'o-', label=altered_parameter)

plt.legend()
plt.savefig(out_dir / 'figS4-equal_reasons_width.svg')
plt.savefig(out_dir / 'figS4-equal_reasons_width.png')
...
plt.legend()
plt.savefig(out_dir / 'figS4-optimal_width.svg')
plt.savefig(out_dir / 'figS4-optimal_width.png')
...
plt.legend()
plt.savefig(out_dir / 'figS4-minimal_event_propensity.svg')
plt.savefig(out_dir / 'figS4-minimal_event_propensity.png')
...
for altered_parameter, data in event_propensity_for_w_6.groupby('altered_parameter'):
    ax.plot(data.index.get_level_values('fold_change'), 1/data, 'o-', label=altered_parameter)

plt.legend()
plt.savefig(out_dir / 'figS4-event_propensity_for_w_6.svg')
plt.savefig(out_dir / 'figS4-event_propensity_for_w_6.png')
