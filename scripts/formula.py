...
def predicted_optimal_interval_formula(channel_width, channel_length, parameters=PARAMETERS_DEFAULT, mol_states=MOL_STATES_DEFAULT):
    return (get_cycle_time(parameters, mol_states) + get_cycle_time_std(parameters, mol_states) + np.e/1.5 * np.sqrt(get_time_var(channel_width) * channel_length))
    return np.sqrt((get_cycle_time(parameters, mol_states) + get_cycle_time_std(parameters, mol_states))**2 + (np.e * np.sqrt(get_time_var(channel_width) * channel_length))**2)
    return (
        (get_cycle_time(parameters, mol_states)# - 15
        + (np.log(channel_length) - .3)/3 * get_cycle_time_std(parameters, mol_states) # np.log(np.sqrt(channel_length)/2)
        + (np.log(channel_length) - .3)/3/2 * np.sqrt(get_time_var(channel_width) * channel_length)) #* get_cycle_time_std(parameters, mol_states)
    )


def predicted_bitrate_formula(channel_width, channel_length, parameters=PARAMETERS_DEFAULT, mol_states=MOL_STATES_DEFAULT):
    return (
        max(get_cycle_time(parameters, mol_states) 
        + np.log(np.sqrt(channel_length)/2) * get_cycle_time_std(parameters, mol_states),
...
def get_factors(channel_width, channel_length, parameters=PARAMETERS_DEFAULT, mol_states=MOL_STATES_DEFAULT):
    ...
    # opt_interval = get_cycle_time(parameters, mol_states) + .5* get_cycle_time_std(parameters, mol_states) + np.e * np.sqrt(get_time_var(channel_width) * channel_length)
    ...
    # factor_refractory = (
    #     (1 - 0.25 * np.e) 
    #     * (get_cycle_time(parameters, mol_states) + np.log(np.sqrt(channel_length)/2) * get_cycle_time_std(parameters, mol_states)) 
    #     / (np.e * np.sqrt(get_time_var(channel_width) * channel_length))
    # )


    temporal_scale = np.e * np.sqrt(get_time_var(channel_width) * channel_length)
    refractory_time = get_cycle_time(parameters, mol_states)
...
def get_length_with_variance_equal_refractory(channel_width, parameters=PARAMETERS_DEFAULT, mol_states=MOL_STATES_DEFAULT):
    return (get_cycle_time(parameters, mol_states) / np.e)**2 / get_time_var(channel_width)
